...
document.addEventListener('DOMContentLoaded', function() {
    originalProductsList = document.querySelectorAll('.product-list .row .col-sm-3');

    var categoryMap = {
        'default': [],
        'Trà sữa truyền thống': ['Trà sữa matcha', 'Trà sữa bạc hà', 'Trà sữa dâu', 'Trà sữa hoàng kim'],
        'Cà phê': ['Bạc xỉu', 'Cafe sữa', 'Cafe đen'],
        'Đá xay': ['Đào hồng đá tuyết', 'Grape Smoothie'],
        'Trà trái cây': ['Trà Alisan Gong Cha Nhãn Sen', 'Grape Green Tea']
    };

    document.getElementById('groupSelect').addEventListener('change', function() {
        var selectedCategory = this.value;
        var products = document.querySelectorAll('.product-list .row .col-sm-3');
        Array.from(products).forEach(function(product) {
            var title = product.querySelector('.card-title').textContent;
            if (selectedCategory === 'default' || categoryMap[selectedCategory].includes(title)) {
